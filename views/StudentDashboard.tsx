                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className={`p-5 md:p-6 rounded-2xl ${isDark ? 'glass-panel' : 'paper-panel'}`}>
                        <div className="flex justify-between items-start mb-4">
                            <h3 className={`font-bold uppercase tracking-widest text-xs ${isDark ? 'text-f-neon' : 'text-h-accent'}`}>{t.wordOfDay}</h3>
                            <button onClick={() => playPronunciation(DAILY_WORD.text)} className={`p-2 rounded-full transition-colors ${isDark ? 'bg-white/5 hover:bg-f-neon hover:text-black' : 'bg-black/5 hover:bg-h-accent hover:text-white'}`}>
                                <iconify-icon icon="solar:volume-loud-bold"></iconify-icon>
                            </button>
                        </div>
                        <div className="mb-4">
                            <span className={`text-2xl font-bold ${isDark ? 'font-future' : 'font-heritage'}`}>{DAILY_WORD.hindiText}</span>
                            <p className="text-xs opacity-60 mt-1">{DAILY_WORD.phonetic} • {DAILY_WORD.type}</p>
                        </div>
                        <p className="text-sm italic opacity-80 leading-relaxed">"{DAILY_WORD.meaning}"</p>
                    </div>
